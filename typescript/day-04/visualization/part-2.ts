...
const removeAccessiblePapers = async (matrix: Matrix<string>) => {
  let papers = getAccessiblePapers(matrix);

  await TerminalVisualization.render(matrix, 500);

  while (papers.length !== 0) {
    matrix = removeMarkedPapers(matrix, papers);
    papers = getAccessiblePapers(matrix);

    await TerminalVisualization.render(matrix);
  }
...
removeAccessiblePapers(matrix).then(countRemovedPapers).then(console.log);
